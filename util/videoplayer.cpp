...
public:
    ...
    Thread(VideoPlayer *player, oz::ffmpeg_decoder *ffmpeg, int cacheRadius, int cacheMin, int cacheMax ) 
...
bool VideoPlayer::open(const QString& path) {
    ...
    oz::ffmpeg_decoder *ffmpeg = oz::ffmpeg_decoder::open(path8.data());
    if (ffmpeg) {
...
void VideoPlayer::setPlayback(bool playing) {
    ...
    if (m_thread && (isPlaying() != playing)) {
        if (playing) {
            oz::ffmpeg_decoder *f = m_thread->m_ffmpeg;
            m_ticks = f->ticks();
...
void VideoPlayer::record() {
    ...
    oz::ffmpeg_encoder *encoder = oz::ffmpeg_encoder::create(filename.toStdString().c_str(), f->width(), f->height(), f->frame_rate()); 
    if (!encoder) {
