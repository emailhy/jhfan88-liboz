...
void oz::gpu_cache_alloc( void **ptr, unsigned *pitch, unsigned w, unsigned h ) {
    ...
    if (i != g_cache.end()) {
        ...
        *pitch = i->second.pitch;
        ...
        size_t tmp;
        cudaError_t err = cudaMallocPitch(ptr, &tmp, w, h);
        if (err != cudaSuccess) {
            gpu_cache_clear();
            err = cudaMallocPitch(ptr, &tmp, w, h);
        }
        if (err != cudaSuccess) {
            OZ_X() << err;
        }
...
void oz::gpu_cache_free( void *ptr, unsigned pitch, unsigned w, unsigned h ) {
...
size_t oz::gpu_cache_size() {
    return g_cache_size;
}


size_t oz::gpu_cache_total() {
    return g_total_size;
}
