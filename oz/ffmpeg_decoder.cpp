...
oz::ffmpeg_decoder* oz::ffmpeg_decoder::open(const char* path) {
...
oz::ffmpeg_decoder::ffmpeg_decoder(impl *p) : m(p) {}


oz::ffmpeg_decoder::~ffmpeg_decoder() {
    delete m;
    m = 0;
}


bool oz::ffmpeg_decoder::next() {
...
void oz::ffmpeg_decoder::rewind() {
    this->set_frame(0);
}


void oz::ffmpeg_decoder::set_time( int64_t ts, bool exact ) {
...
void oz::ffmpeg_decoder::set_frame( int frame, bool exact ) {
...
bool oz::ffmpeg_decoder::at_end() const {
    return m->done;
}


int oz::ffmpeg_decoder::width() const {
    return m->width;
}


int oz::ffmpeg_decoder::height() const {
    return m->height;
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::duration() const {
    return m->stream->duration;
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::start_time() const {
    return m->pts[0];
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::end_time() const {
    return m->pts[0] + m->stream->duration;
}


int oz::ffmpeg_decoder::frame_count() const {
    return (int)m->pts.size();
}


oz::ffmpeg_decoder::rational oz::ffmpeg_decoder::frame_rate() const {
    return rational(m->stream->r_frame_rate.num, m->stream->r_frame_rate.den);
}


oz::ffmpeg_decoder::rational oz::ffmpeg_decoder::time_base() const {
    return rational(m->stream->time_base.num, m->stream->time_base.den);
}


oz::ffmpeg_decoder::uint8_t* oz::ffmpeg_decoder::buffer() const {
    return m->buffer;
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::current_time() const {
    return m->currentTime;
}


int oz::ffmpeg_decoder::current_frame() const {
    return m->currentFrame;
}


double oz::ffmpeg_decoder::fps() const {
    double delta = (double)m->stream->duration / m->pts.size();
    return (double)m->stream->time_base.den / (double)m->stream->time_base.num / delta;
}


int oz::ffmpeg_decoder::frame_from_time(int64_t ts) const {
...
oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::time_from_frame(int frame) const {
    frame = std::min<int>( std::max<int>(0, frame), (int)m->pts.size() - 1 );
    return m->pts[frame];
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::ticks() const {
    return av_gettime();
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::ticks_from_time(int64_t x) const {
    AVRational Q = {1, AV_TIME_BASE};
    return av_rescale_q(x, m->stream->time_base, Q);
}


oz::ffmpeg_decoder::int64_t oz::ffmpeg_decoder::time_from_ticks(int64_t t) const {
    AVRational Q = {1, AV_TIME_BASE};
...
/*
    ff_print_debug_info()
    m->codecCtx->debug_mv = 1;
    frame.debug |= FF_DEBUG_MV
*/
